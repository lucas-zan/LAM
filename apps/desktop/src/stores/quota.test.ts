...
describe('useQuotaStore', () => {
  ...
  it('surfaces backend quota warnings without dropping successful snapshots', async () => {
    ...
    expect(useAppStore.getState().status).toBe('Refreshed 1 quota snapshots; 1 unavailable');
    expect(useAppStore.getState().error).toBe('a: realtime quota unavailable; using cached quota');
  });

  it('clears stale quota warnings after a fresh account refresh', async () => {
    useAppStore.getState().setError('c: realtime quota unavailable; using cached quota');
